...
function robotMove(cFrom,cTo){
	var http = new XMLHttpRequest();
	var params = 'from=' + cFrom + '&to=' + cTo;
	http.open('GET', 'http://192.168.1.1/move.html?' + params, true);
	http.send();
...
const startBoard = game => {
    ...
    game.on('pieceMove', piece => {
		robotMove(document.getElementById(piece.name).parentElement.id,piece.position);
    ...
    game.on('kill', piece => {
        const pieceImg = document.getElementById(piece.name);
		robotMove(document.getElementById(piece.name).parentElement.id,00);
